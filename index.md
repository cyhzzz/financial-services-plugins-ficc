# FICC Financial Services Plugins

基于 [Anthropic financial-services-plugins](https://github.com/anthropics/financial-services-plugins) 架构的商业银行 FICC（固定收益、外汇、大宗商品）业务完整技能体系。

## 🏗️ 架构设计

本项目严格遵循 Anthropic financial-services-plugins 的插件化架构：
...
```bash
# 利率债分析
/fixed-income gov-bond --type国债 --maturity 10y --yield 2.65%


# 信用债分析
/fixed-income credit-bond --issuer城投A --rating AA+ --province 江苏 --debt-ratio 180% --spread 85bp

# 收益率曲线分析
/fixed-income yield-curve --build --tenor 1y-30y

# IRS 定价
/fixed-income irs-pricing --notional 1亿 --tenor 5Y --fixed-rate 2.85% --float-leg 3M-Shibor --valuation OIS-discounting
